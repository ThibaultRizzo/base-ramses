...
class RamsesModel(SimpleTradingModel):
    ...
    def compute_signals(cls, df, features: DataFrame):
        return DataFrame({"rsi": compute_rsi_signals(features["rsi"]), "breakout": compute_breakout_signals(df)})
    ...
    def _get_position_state(cls, df, stop_loss, stop_profit):
        ...
        for i in range(1, df.shape[0]):
            ...
            if df["LONG"][i] and last_position != 1:
                ...
                model_positions.append(
                    (0, None, df["Close"][i], cls._get_exit_price(model_positions, df["EXIT_LONG"][i]))
                )
        ...
        for i in range(1, df.shape[0]):
            ...
            if is_long and df["EXIT_LONG"][i]:
                ...
                state = (
                    State.EXIT_LONG_SUR_STOP_PROFIT if not df["SHORT"][i] else State.SHORT_AND_EXIT_LONG_SUR_STOP_PROFIT
                )
                ...
                state = (
                    State.EXIT_SHORT_SUR_STOP_PROFIT if not df["LONG"][i] else State.LONG_AND_EXIT_SHORT_SUR_STOP_PROFIT
                )
            elif position == 1:
                state = State.LONG
            elif position == -1:
            ...
            state_list.append(
                {
                    ...
                    if state in [State.LONG, State.SHORT, State.NONE]
                    else model_positions[i - 2][1],
