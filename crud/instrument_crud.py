...
from models import HourlyPriceTimeseries, Instrument
...
class InstrumentCrud(BaseCrud[Instrument]):
    ...
    def get_timeseries(cls, session, code: str, start_date: datetime, end_date: datetime):
        ...
        instrument = cls.get_one(session, (Instrument.code == code,))
        if not instrument:
            raise Exception(f"No instrument found for code {code}")

        timeseries = HourlyPriceTimeseriesCrud.get_all(
            session,
            (
        ...
        needed_dates = [
            datetime.combine(start_date.date(), expected_start_time) + timedelta(days=day, hours=hour)
            for day in range(datetime_delta.days + 1)
            for hour in range(expected_end_time.hour + 1 - expected_start_time.hour)
        ]
