...
#####################################################
####### Stratégies choisies
sample_strategies = [RMSlib.strategy_Breakout, RMSlib.strategy_RSI]
#####################################################
####### Paramètres des stratégies
##  pour chaque stratégie, on crée un jeu de paramètres
strat_0 = {"strat": sample_strategies[0], "param": {"n_up": [15, 25, 35], "n_down": [11, 21, 31], "n_shift": [0, 1]}}
strat_1 = {
    "strat": sample_strategies[1],
    "param": {"n": [i for i in range(9, 22, 3)], "n_seuil": [i for i in range(15, 31, 5)], "n_shift": [0, 1]},
}
...
for strategy in strategies:
    ...
    trade_statistics_global = trade_stat_global.groupby(["pos", "Win_Loss"]).agg(["count", "mean", "max", "min"])
    trade_statistics_global = trade_statistics_global.reset_index()
    trade_statistics_global.columns = [
        "pos",
        "Win_Loss",
        "N_trades",
        "Avg_ndays",
    ...
    win_ratio = trade_statistics_global[trade_statistics_global.Win_Loss == "Win"].N_trades.sum() / tot_number_trades
