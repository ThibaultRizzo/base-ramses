...
def test_ramses_model_compute_signals():
    ...
    print(RamsesModel.compute_signals(df, RamsesModel.compute_features(df)))
...
def test_ramses_model_get_position_state():
    ...
    df = DataFrame(
        ...
        index=DatetimeIndex([datetime(2022, 1, 1), datetime(2022, 1, 2), datetime(2022, 1, 3), datetime(2022, 1, 4)]),
    )
    compare_list(
        [
            {
        ...
        RamsesModel._get_position_state(df, 0.05, 0.02),
    )
    ...
    compare_list(
        ...
        RamsesModel._get_position_state(df, 0.5, 0.5),
    )
